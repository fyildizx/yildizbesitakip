...
// script.js dosyasının en altına
async function giderKaydet() {
    const kategori = document.getElementById('gider-kategori').value;
    const ad = document.getElementById('gider-ad').value;
    const miktar = parseFloat(document.getElementById('gider-miktar').value);
    const birimFiyat = parseFloat(document.getElementById('gider-birim-fiyat').value);
    const tarih = document.getElementById('gider-tarih').value;

    if(!ad || !miktar || !birimFiyat || !tarih) {
    ...
    try {
        await db.collection("giderler").add({
            kategori: kategori,
            ad: ad,
            miktar: miktar,
        ...
        alert("Gider başarıyla işlendi! Toplam: " + toplam.toFixed(2) + " TL");
        
        // Formu temizle
        document.getElementById('gider-ad').value = "";
        document.getElementById('gider-miktar').value = "";
        document.getElementById('gider-birim-fiyat').value = "";
    } catch (error) {
        console.error("Gider kaydedilirken hata oluştu: ", error);
        alert("Bir hata oldu, bağlantıyı kontrol et.");
    }
}
